(() => {
  ...
  function sourceSummary(agency, source) {
    ...
    const newest = source.newestRecallDate ? `newest listed recall ${formatDate(source.newestRecallDate)}` : "newest recall date unavailable";
    if (source.success) return `${agency}: checked successfully ${formatDate(source.checkedAt || source.retrievedAt, true)} · ${newest}`;
    const lastGood = source.lastSuccessfulUpdate || source.retrievedAt;
    return `${agency}: latest check failed · last successful retrieval ${formatDate(lastGood, true)} · ${newest}`;
  }

  function setHomepageSummary(data) {
    ...
    const bothCurrent = Boolean(fda?.success && usda?.success);
    ...
    if (footer) {
      footer.textContent = `Official data check: ${formatDate(data?.dataHealth?.checkedAt || data?.generatedAt, true)}.`;
    }
  ...
  function setListingSummary(data) {
    ...
    checked.textContent = `Official sources checked ${formatDate(data?.dataHealth?.checkedAt || data?.generatedAt, true)}.`;
    root.append(checked);
