(function (global) {
  ...
  function signalText(record) {
    ...
    const values = [
      ...
      ...(record?.categories || []),
      ...(record?.hazards || []),
      source.field_recall_type,
      source.recall_type,
      source.product_type,
      source.product_types,
      source.category,
      source.recall_reason_description,
      source.reason_for_announcement,
  ...
  const CATEGORY_RULES = {
    ...
    "dairy-eggs": r => hasAny(signalText(r), [
      /\bdairy\b/, /\bmilk\b/, /\bcheese\b/, /\bcream\b/, /\byogurt\b/, /\bbutter\b/, /\begg(s)?\b/
    ]),
    "prepared-foods": r => hasAny(signalText(r), [
      /\bprepared food(s)?\b/, /\bready[- ]to[- ]eat\b/, /\brte\b/, /\bmeal(s)?\b/, /\bsandwich(es)?\b/,
      /\bsalad(s)?\b/, /\bsoup(s)?\b/, /\bpizza(s)?\b/, /\bwrap(s)?\b/, /\bbowl(s)?\b/, /\bentree(s)?\b/
    ]),
    allergens: r => hasAny(signalText(r), [
      /\ballergen(s)?\b/, /\bunreported allergen(s)?\b/, /\bundeclared\b/, /\bmisbranding\b.*\ballergen/, /\bmilk allergen\b/, /\begg allergen\b/,
      /\bpeanut(s)?\b/, /\bsoy\b/, /\bwheat\b/, /\bsesame\b/, /\b tree nut(s)?\b/, /\bshellfish\b/
    ]),
    contamination: r => hasAny(signalText(r), [
      /\bproduct contamination\b/, /\bcontamination\b/, /\bcontaminated\b/, /\bsalmonella\b/, /\blisteria\b/,
      /\be\.?\s*coli\b/, /\bclostridium\b/, /\bbotulism\b/, /\bforeign material\b/, /\bforeign matter\b/
    ])
  };
  ...
  function currentRecalls(records) {
    return (records || [])
      .filter(r => actionable(r) && !CLOSED.has(lifecycle(r)))
      .sort((a,b) => Date.parse(b.recallDate || b.timeline?.recallDate || 0) - Date.parse(a.recallDate || a.timeline?.recallDate || 0));
  }

  function reliableCategories(record) {
    return Object.entries(CATEGORY_RULES).filter(([,test]) => test(record)).map(([key]) => key);
  }

  function filterRecalls(records, filter) {
    const list = currentRecalls(records);
    if (!filter || filter === "all") return list;
    if (["FDA", "USDA"].includes(filter)) return list.filter(r => r.agency === filter);
    return CATEGORY_RULES[filter] ? list.filter(CATEGORY_RULES[filter]) : [];
  }
  ...
  function validRecallId(value) {
    return typeof value === "string" && value.length > 0 && value.length <= 160 && /^[A-Za-z0-9._:-]+$/.test(value);
  }

  function detailUrl(id) {
    return validRecallId(id) ? `recall.html?id=${encodeURIComponent(id)}` : "recalls.html";
  }

  function safeOfficialUrl(value) {
    try {
      const u = new URL(value);
      return u.protocol === "https:" && OFFICIAL_HOSTS.some(h => u.hostname === h || u.hostname.endsWith(`.${h}`)) ? u.href : "";
    } catch (_) { return ""; }
  }
  ...
  function formatDate(value) {
    ...
    return Number.isFinite(t)
      ? new Intl.DateTimeFormat("en-US", {year:"numeric", month:"long", day:"numeric", timeZone:"UTC"}).format(t)
      : "Not listed";
  }

  function text(tag, value, className) {
    const n = document.createElement(tag);
    n.textContent = value == null ? "Not listed" : value;
  ...
  function renderCard(record) {
    ...
    const meta = text("p", `${record.agency} · ${formatDate(record.recallDate || record.timeline?.recallDate)} · ${status}`, "recall-meta");
    a.append(meta, text("p", record.reason || "See the official notice for details."));
    const cats = reliableCategories(record);
    if (cats.length) a.append(text("p", cats.map(categoryLabel).join(" · "), "category"));
    const link = text("a", "View recall", "secondary");
    link.href = detailUrl(record.id);
    a.append(link);
    return a;
  }

  async function load() {
    const response = await fetch("data/recalls.json", {cache:"no-store"});
    if (!response.ok) throw new Error("Recall data unavailable");
  ...
  async function recent() {
    ...
    try {
      ...
      root.replaceChildren(...currentRecalls(data.recalls).slice(0,3).map(renderCard));
    } catch (_) {
      root.replaceChildren(text("p", "Recent recalls could not be loaded. Visit the official FDA or USDA recall pages."));
    }
  }
  ...
  async function listing() {
    ...
    if (!root) return;
    let records = [];
    try { records = (await load()).recalls; }
    catch (_) { root.replaceChildren(text("p", "Current recalls could not be loaded.")); return; }
    const buttons = [...document.querySelectorAll("[data-filter]")];
    const draw = filter => {
      const shown = filterRecalls(records, filter);
      root.replaceChildren(...shown.map(renderCard));
      if (!shown.length) root.append(text("p", "No current recalls match this filter."));
  ...
  async function detail() {
    const root = document.querySelector("[data-recall-detail]");
    if (!root) return;
    const id = new URLSearchParams(location.search).get("id");
    if (!validRecallId(id)) return notFound(root);
    try {
      const data = await load(), r = data.recalls.find(x => x.id === id);
      if (!r) return notFound(root);
      ...
      bannerCopy.append(text("p", `${status} · ${r.agency}`, "result-label"), text("h1", r.title || r.productDescription || "Recall details", "result-heading"));
      banner.append(bannerCopy);
      const summary = text("section", "", "result-summary");
      summary.append(text("p", current ? "Do not eat or serve this product until you review the official recall instructions." : "The official record is listed as closed or terminated. This page does not identify a current package as part of an active recall.", "result-instruction"), text("h2", "Affected package"), text("p", [...(r.lotCodes || []), ...(r.dateCodes || []), ...(r.packageSizes || []), ...(r.establishmentNumbers || [])].join(", ") || r.productDescription || "See the official notice."));
      ...
      [["Recall reason",r.reason],["Recalling company",r.recallingFirm],["Agency",r.agency],["Recall date",formatDate(r.recallDate || r.timeline?.recallDate)],["Official status",cleanSourceLabel(r.lifecycle?.sourceStatus || r.status)],["Distribution",r.distribution],["Recall data updated",formatDate(data.generatedAt)]].forEach(([k,v]) => dl.append(text("dt",k), text("dd",v)));
      details.append(dl);
